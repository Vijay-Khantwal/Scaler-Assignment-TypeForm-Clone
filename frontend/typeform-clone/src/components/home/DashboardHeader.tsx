...
export function DashboardHeader({ activeTab = 'forms' }: DashboardHeaderProps) {
  return (
    <header className="flex flex-col bg-white border-b border-[#e4e4e7] shrink-0 z-20">
      {/* Top row */}
      <div className="flex items-center justify-between px-4 h-12 border-b border-[#f0eeef]">
        {/* Left: workspace name */}
        <div className="flex items-center gap-2">
          ...
          <button className="flex items-center gap-1 text-sm font-medium text-[#3C323E] hover:text-[#1a1a1a] transition-colors">
            vkhantwal999
        ...
        <div className="flex items-center gap-1">
          <button className="flex items-center gap-1.5 px-2.5 py-1.5 text-xs text-[#655D67] hover:bg-[#f7f5f8] rounded-lg transition-colors">
          ...
          <button className="flex items-center gap-1.5 px-3 py-1.5 text-sm font-medium text-white bg-[#196042] hover:bg-[#145236] rounded-lg transition-colors">
            View plans
          </button>
          <button className="p-1.5 text-[#847E85] hover:text-[#655D67] hover:bg-[#f7f5f8] rounded-lg transition-colors ml-0.5">
            <IconHelp size={15} />
          </button>
          {/* User avatar */}
          <div className="w-7 h-7 rounded-full bg-[#C68642] flex items-center justify-center ml-1 cursor-pointer">
      ...
      <div className="flex items-center px-5 h-10">
        {TABS.map((tab) => (
          <button
            key={tab.id}
            className={cn(
              'flex items-center gap-1.5 px-1 h-full text-sm transition-colors relative mr-1',
              'border-b-2 -mb-px',
              activeTab === tab.id
                ? 'text-[#3C323E] border-[#3C323E] font-medium'
                : 'text-[#655D67] border-transparent hover:text-[#3C323E] hover:border-[#c8c4c9]'
            )}
          >
            {tab.icon && <span className="shrink-0">{tab.icon}</span>}
            {tab.label}
            {tab.badge && (
              <PillBadge label={tab.badge} className="ml-0.5" />
            )}
          </button>
        ))}
      </div>
